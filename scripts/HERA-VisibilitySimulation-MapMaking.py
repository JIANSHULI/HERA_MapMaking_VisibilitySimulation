...
Frequency_Min = 80. if 'blender' in DATA_PATH else 80.0
Frequency_Max = 81. if 'blender' in DATA_PATH else 81.0
...
for id_File_Loop, File_Loop in enumerate(np.arange(File_Start, File_End, File_Step)):
    ...
    for id_Frequency_Select, Frequency_Select in enumerate(np.arange(Frequency_Min, Frequency_Max, Frequency_Step)):
        ...
        filetype = 'miriad' if INSTRUMENT == 'hera' else 'uvh5' if INSTRUMENT == 'hera-vivaldi' else 'uvh5'  # if 'blender' in DATA_PATH else 'uvh5'  # 'miriad', 'uvh5'
        ...
        baseline_safety_xx = 7. if INSTRUMENT == 'hera' else 0. if INSTRUMENT == 'hera-vivaldi' else 7.  # Meters
        baseline_safety_yy = 7. if INSTRUMENT == 'hera' else 0. if INSTRUMENT == 'hera-vivaldi' else 7.  # Meters
        baseline_safety_zz = 0. if INSTRUMENT == 'hera' else 0. if INSTRUMENT == 'hera-vivaldi' else 0.
        baseline_safety_xx_max = 7000. if INSTRUMENT == 'hera' else 10. ** 5 if INSTRUMENT == 'hera-vivaldi' else 70.  # 10.**10  # Meters
        baseline_safety_yy_max = 7000. if INSTRUMENT == 'hera' else 10. ** 5 if INSTRUMENT == 'hera-vivaldi' else 70.  # 10.**10  # Meters
        ...
        Artifitial_GSM = True if ('blender' in DATA_PATH and Simulation_For_All) else True if Simulation_For_All else False  # If to use artifitail GSM.
        Artifitial_uniform_sky_T = 0.5  # K
        T_receiver = 0.  # K; 100.
        INSTRUMENT += ('-AT{0}K'.format(Artifitial_uniform_sky_T) if Artifitial_GSM else '')
        ...
        Delay_Filter = False
        Observing_Session = (['/IDR2_1/LSTBIN/one_group/grp1/'] if LST_binned_Data else ['/IDR2_1/2458105/']) if filetype != 'uvh5' else ['/ObservingSession1232039492/2458504/'] if Use_External_Vis else [
        ...
        if len(sys.argv) > 2:
            ...
            Specific_FileIndex_start = ([13 for id_p in range(Num_Pol)] if LST_binned_Data else [15 for id_p in range(Num_Pol)]) if filetype == 'miriad' else [35 for id_p in range(Num_Pol)] if filetype == 'uvh5' else [0 for id_p in range(Num_Pol)]  # [3, 3]  # Starting point of selected data sets. [51, 51], 113:[26, 27], 105:[28, 29]; [15, 65], [6,32]
            Specific_FileIndex_end = ([18 for id_p in range(Num_Pol)] if LST_binned_Data else [60 for id_p in range(Num_Pol)]) if filetype == 'miriad' else [95 for id_p in range(Num_Pol)] if filetype == 'uvh5' else [40 for id_p in range(Num_Pol)]  # [23, 23]  # Ending point of selected data sets. [51, 51], [26, 27]
        else:
        ...
        if len(sys.argv) > 4:
            ...
            nside_start = 32 if ('blender' in DATA_PATH and Simulation_For_All) else 32 if ('blender' in DATA_PATH and not Simulation_For_All) else 32  # starting point to calculate dynamic A
            nside_standard = 32 if ('blender' in DATA_PATH and Simulation_For_All) else 32 if ('blender' in DATA_PATH and not Simulation_For_All) else 32  # resolution of sky, dynamic A matrix length of a row before masking.
        ...
        if nchunk_from_memory_calculation_full and Parallel_A_fullsky:
            ...
            memory_left = - resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024. ** 3 + virtual_memory().total / 1024. ** 3
            print('\n>>>>>>>>>>> A_size: {0} GB; Memory Left: {1} GB.'.format(A_size_memory, memory_left))
            nchunk_A_full = np.int(A_size_memory / (memory_left * 0.96 / 3.)) + 1
            print('>>>>>>>>>>> New nchunk from memory calculation: {0} \n'.format(nchunk_A_full))
            if nchunk_A_full > 1:
        ...
        if nchunk_from_memory_calculation:
            ...
            if np.int(A_size_memory / (memory_left * 0.9 / 2.) + 1) <= 1:
                ...
                nchunk = (np.int(A_size_memory / (memory_left * 1 / 3)) + 1) if 'blender' in DATA_PATH else (np.int(A_size_memory / (memory_left * 0.9 / 3. + 1)))
            print('>>>>>>>>>>> New nchunk from memory calculation: {0} \n'.format(nchunk))
        ...
        id_primary = (thetas_ps < (128. / 12. * np.pi)) * (thetas_ps > (112. / 180. * np.pi)) * (phis_ps > lsts.min() / 12. * np.pi) * (phis_ps < lsts.max() / 12. * np.pi)
