...
for p, pol in enumerate(['xx', 'xy', 'yx', 'yy']):
    ...
    for t in range(len(omnifits['xx'])):
        ...
        d[flag[t]] = 0
        compr_result = omni.deconvolve_spectra2(d, ~flag[t], (delay_compression+1)/2, correction_weight=1e-6)
        ...
        if p == 0:
            ...
            compr_flag[t,compr_error_bar > 5.5 * float(delay_compression)/nfreq] = True#min(compr_error_bar) * 1.05] = True
...
################################
calibrated_data = np.empty_like(compressed_data[:,~compr_flag[:, pick_f],pick_f])
for p, pol in enumerate(['xx', 'xy', 'yx', 'yy']):
    original_data = np.copy(compressed_data[p,~compr_flag[:, pick_f],pick_f])
    calibrated_data[p] = original_data * (ampcals[pol[0]*2] * ampcals[pol[1]*2])**.5 * np.exp(1.j * info['ubl'][:,:2].dot(phase_cal))
...
for i in range(info['nUBL']):
    ...
    plt.scatter((lsts-3)%TPI+3,np.imag(calibrated_data[p,:,u]))
    plt.title(info['ubl'][u]);plt.xlim(4.5,6.5)
plt.show()
