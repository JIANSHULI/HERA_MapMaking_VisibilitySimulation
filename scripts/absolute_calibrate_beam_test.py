...
bnside = 256
local_beam_data = np.concatenate([np.fromfile('/home/omniscope/data/mwa_beam/healpix_%i_%s.bin'%(bnside,p), dtype='complex64').reshape((9,12*bnside**2,2)) for p in ['x', 'y']], axis=-1).transpose(0,2,1)

for y_factor in np.arange(.5, 1.5, .1):
    local_beam = si.interp1d(range(110,200,10), local_beam_data * np.array([1, y_factor, y_factor, 1])[None, :, None], axis=0)
    ...
    for pick_f in psols.keys():
        ...
        #
    np.savez(datadir + 'cygcas_' + Q + datatag + vartag + '_crossbeam%.3f'%y_factor, cyg_cas_iquv=cyg_cas_iquv, cyg_cas_iquv_std=cyg_cas_iquv_std, freqs=freqs)
