...
File_Width = 45 # Number of files loaded into each iteration.

Lsts_List_start = -0.5 # -12.5
Lsts_List_end = 5.5 # 12.5
Lsts_List_step = 100. # 2.5
Lsts_Width = 1.
...
nside_beamweight = 8  # undynamic A matrix shape

Time_Average_preload = 1  # 12 # Number of Times averaged before loaded for each file (keep tails)'
Frequency_Average_preload = 50  # 16 # Number of Frequencies averaged before loaded for each file (remove tails)'

Valid_Threshold = 10**(-6.)

for freq in np.arange(Frequency_Min, Frequency_Max, Frequency_Step):
    if freq < 140.:
        nside_start = 32 # 128
        nside_standard = 32 # 128
        nside_beamweight = 8 # 32
    else:
        nside_start = 32 # 256
        nside_standard = 32 # 256
        nside_beamweight = 8 # 64
