...
def UVData2AbsCalDict_Auto(datanames, pol_select=None, pop_autos=True, return_meta=False, filetype='miriad',
	...
	if Xrfi_flag:
		print('\n Redo Xrfi with Kt={0}, Kf={1}, sig_init={2}, sig_adj={3}, px_threshold={4}, freq_threshold={5}, time_threshold={6}, cal_mode={7}. \n'.format(Kt, Kf, sig_init, sig_adj, px_threshold, freq_threshold, time_threshold, cal_mode))
		
		timer_xrfi = time.time()
		...
		print('Time used for Redoing Xrfi: {0} minutes \n'.format((time.time() - timer_xrfi) / 60.))
...
def De_Redundancy(dflags=None, antpos=None, ants=None, SingleFreq=True, MultiFreq=True, Conjugate_CertainBSL=False, Conjugate_CertainBSL2=False, Conjugate_CertainBSL3=False, data_freqs=None, Nfreqs=64, data_times=None, Ntimes=None, FreqScaleFactor=1.e6, Frequency_Select=None, Frequency_Select_Index=None,
	...
	print('\n total_max={9}, xx_min={0}, yy_min={1}, zz_min={2}, \n xx_max={3}, yy_max={4}, zz_max={5}\n Tolerance:{6}, RFI_Free_Thresh={7}, RFI_Free_bslStrength={8}'.format(baseline_safety_xx, baseline_safety_yy, baseline_safety_zz, baseline_safety_xx_max, baseline_safety_yy_max, baseline_safety_zz_max, tol, RFI_Free_Thresh, RFI_Free_Thresh_bslStrengthen,
																																											  (C / freq) * 1.4 * nside_standard / baseline_safety_factor))
	...
	for i in range(Num_Pol):
		...
		print('Ubl_list[{0}]: {1}'.format(i, Ubl_list[i]))
		...
		if Parallel_A:
			...
			# print('Shape of A after reshaping: {0}'.format(A.shape))
				...
				if Synthesize_MultiFreq:
					...
					A = np.array([[[vs.calculate_pointsource_visibility(hpf.pix2ang(nside_beamweight, n)[1], (np.pi / 2. - hpf.pix2ang(nside_beamweight, n)[0]), used_common_ubls, f, None, beam_heal_equ[id_p], None, lsts) * (equatorial_GSM_beamweight_mfreq_nest[Flist_select_index[id_p][id_f], n] / equatorial_GSM_beamweight_nest[n]) * 0.5 for n in range(12 * nside_beamweight ** 2)] for id_f, f in enumerate(Flist_select[id_p])] for id_p in range(2)], dtype=Precision_full).transpose(
						(0, 1, 3, 4, 2)).reshape(2, len(Flist_select[id_p]) * len(used_common_ubls) * nt_used, 12 * nside_beamweight ** 2)
				else:
					if beam_heal_equ_x is None:
						try:
							beam_heal_equ_x = beam_heal_equ_x_mfreq[Reference_Freq_Index[0]]
					...
					if Use_memmap_A_full:
						A = np.memmap(A_path_full, dtype=Precision_full, mode='w+', order=A_chunk_order, shape=(Num_Pol, len(Flist_select[id_p]), len(used_common_ubls), nt_used, 12 * nside_beamweight ** 2))
						if not Array_Pvec:
							A[:] = 0.5 * np.array([[[vs.calculate_pointsource_visibility(hpf.pix2ang(nside_beamweight, n)[1], (np.pi / 2. - hpf.pix2ang(nside_beamweight, n)[0]), used_common_ubls, f, None, beam_heal_equ[id_p], None, lsts) for n in range(12 * nside_beamweight ** 2)] for id_f, f in enumerate(Flist_select[id_p])] for id_p in range(Num_Pol)], dtype=Precision_full).transpose((0, 1, 3, 4, 2)).reshape(Num_Pol, len(Flist_select[id_p]) * len(used_common_ubls) * nt_used,
																																																																																																							  12 * nside_beamweight ** 2)
						else:
							A[:] = 0.5 * np.array([[vs.calculate_pointsource_visibility(hpf.pix2ang(nside_beamweight, np.arange(12 * nside_beamweight ** 2))[1], (np.pi / 2. - hpf.pix2ang(nside_beamweight, np.arange(12 * nside_beamweight ** 2)[0])), used_common_ubls, f, None, beam_heal_equ[id_p], None, lsts) for id_f, f in enumerate(Flist_select[id_p])] for id_p in range(Num_Pol)], dtype=Precision_full).transpose((0, 1, 3, 4, 2)).reshape(Num_Pol,
																																																																																																														 len(Flist_select[id_p]) * len(used_common_ubls) * nt_used,
																																																																																																														 12 * nside_beamweight ** 2)
					else:
						if not Array_Pvec:
							...
							A = np.array([[vs.calculate_pointsource_visibility(hpf.pix2ang(nside_beamweight, np.arange(12 * nside_beamweight ** 2))[1], (np.pi / 2. - hpf.pix2ang(nside_beamweight, np.arange(12 * nside_beamweight ** 2)[0])), used_common_ubls, f, None, beam_heal_equ[id_p], None, lsts) for id_f, f in enumerate(Flist_select[id_p])] for id_p in range(Num_Pol)], dtype=Precision_full).transpose((0, 1, 3, 4, 2)).reshape(Num_Pol, len(Flist_select[id_p]) * len(used_common_ubls) * nt_used,
																																																																																																												12 * nside_beamweight ** 2)
							A *= 0.5
		...
		# return A
			...
			# 		print('A not saved.')
				...
				if A_Method_leg:
					...
					print('Total time used for A: {0} seconds; {1} minutes\n'.format(time.time() - timer, (time.time() - timer) / 60.))
		...
		if NoA_Out or (ChunkbyChunk_all and id_chunk_i == 0):
			...
			for nchunk_A_valid in range(1, 10):
				try:
					...
					if not Special_ReOrder:
						...
						print('Mean of Csim_data: {0}\n'.format(np.mean(Csim_data)))
			...
			if not ChunkbyChunk_all:
				if os.path.isfile(AtNiA_path) and not force_recompute:
					...
					if Parallel_AtNiA:
						...
						if nchunk != 1:
							...
							try:
								...
								sqrNi = ne.evaluate('sqrt(Ni)')
								A *= sqrNi.reshape(len(sqrNi), 1)
								AtNiA[:] = np.dot(A.transpose(), A)
								A /= sqrNi.reshape(len(sqrNi), 1)
							except MemoryError:
								nchunk = nchunk_AtNiA
								print("Allocating AtNiA...")
					...
					print(" \n >>>>>>>>>>>>>> {0} minutes used for AtNiA multiplication <<<<<<<<<<<<<<< \n ".format(float(time.time() - timer) / 60.))
					sys.stdout.flush()
					try:
						del (A)
...
def Flux_Freqfrom150MHz_Spidx(catalog='TGSS', freq=150., flux_mode='Total',
                              flux_threshold=5.,
                              RA_min=0., RA_max=360.,
	...
	#     TGSSADR_catalog = fits.open(DATA_PATH + '/TGSSADR_catalog/TGSSADR1_7sigma_catalog.fits')
	TGSSADR_SpecIndex_catalog = fits.open(DATA_PATH + '/TGSSADR_catalog/spidxcat_v1.1b.fits')
	# Manually add Fornax A (NGC 1316)
	
	stripe_hera = (TGSSADR_SpecIndex_catalog[1].data['Total_flux_{0}'.format(catalog)] > flux_threshold) * (TGSSADR_SpecIndex_catalog[1].data['DEC'] < DEC_max) * (TGSSADR_SpecIndex_catalog[1].data['DEC'] > DEC_min) * (TGSSADR_SpecIndex_catalog[1].data['RA'] > RA_min) * (TGSSADR_SpecIndex_catalog[1].data['RA'] < RA_max)
	print('\nRA_min:{0}; RA_max:{1}; DEC_min:{2}; DEC_Max:{3}'.format(RA_min, RA_max, DEC_min, DEC_max))
	print('Flux_Threshold: {0} Jy'.format(flux_threshold))
	print('Number of Point Sources (Fornax A added): {0}\n'.format(np.sum(stripe_hera) + 1))
	# Get the location in degrees
	RA = TGSSADR_SpecIndex_catalog[1].data["RA"][stripe_hera].astype(np.float)
	DEC = TGSSADR_SpecIndex_catalog[1].data["DEC"][stripe_hera].astype(np.float)
	RA, DEC = np.append(RA, 50.673825), np.append(DEC, -37.208227)
	
	# Get flux in Jy, Jy/beam
	peak_flux = TGSSADR_SpecIndex_catalog[1].data["Peak_flux_{0}".format(catalog)][stripe_hera].astype(np.float)
	tot_flux = TGSSADR_SpecIndex_catalog[1].data["Total_flux_{0}".format(catalog)][stripe_hera].astype(np.float)
	peak_flux, tot_flux = np.append(peak_flux, 7.5 * 10. ** 2), np.append(tot_flux, 7.5 * 10. ** 2)
	
	# Get Spectrum Index
	spectrum_index = TGSSADR_SpecIndex_catalog[1].data["Spidx"][stripe_hera].astype(np.float)
	spectrum_index = np.append(spectrum_index, -0.77)
	
	# Get flux at other frequencies
	if type(freq) == float or type(freq) == int:
		freq = np.array([freq])
	if flux_mode == 'Total':
		flux_freqs = tot_flux * np.array(
			[np.ones(len(spectrum_index)) * (i_freq / 150.) for i_freq in freq]) ** spectrum_index
	elif flux_mode == 'Peak':
		flux_freqs = peak_flux * np.array(
			[np.ones(len(spectrum_index)) * (i_freq / 150.) for i_freq in freq]) ** spectrum_index
	
	data = pandas.DataFrame({"RA": RA, "DEC": DEC,
	                         "Peak flux_{0}".format(catalog): peak_flux,
	                         "Total flux_{0}".format(catalog): tot_flux,
	                         "Spidx": spectrum_index,
	                         })
	print('Shape of flux_freqs: {0}'.format(flux_freqs.shape))
	print('freq:\n{0}'.format(freq))
	if return_table:
		return flux_freqs, data
	else:
		return flux_freqs
...
File_End = 41 if Part_LST else 1
Num_Files_pl = 3 if Part_LST else 1
File_Step = 1 if Part_LST else 1
...
for id_File_Loop, File_Loop in enumerate(np.arange(File_Start, File_End, File_Step)):
	...
	for id_Frequency_Select, Frequency_Select in enumerate(np.arange(Frequency_Min, Frequency_Max, Frequency_Step)):
		...
		try:
			...
			Kf = 80 if INSTRUMENT == 'hera-vivaldi' else 32 if INSTRUMENT == 'hera' else 32  # 32 # 8
			sig_init = 6. if INSTRUMENT == 'hera-vivaldi' else 6. if INSTRUMENT == 'hera' else 6.
			sig_adj = 2. if INSTRUMENT == 'hera-vivaldi' else 2. if INSTRUMENT == 'hera' else 2.
			px_threshold = 0.2 if INSTRUMENT == 'hera-vivaldi' else 0.2 if INSTRUMENT == 'hera' else 0.2
			freq_threshold = 0.25 if INSTRUMENT == 'hera-vivaldi' else 0.25 if INSTRUMENT == 'hera' else 0.1  # 0.1 # 0.5
			time_threshold = 0.025 if INSTRUMENT == 'hera-vivaldi' else 0.025 if INSTRUMENT == 'hera' else 0.01  # 0.01 # 0.05
			return_summary = False if INSTRUMENT == 'hera-vivaldi' else False if INSTRUMENT == 'hera' else False
			cal_mode = 'gain' if INSTRUMENT == 'hera-vivaldi' else 'gain' if INSTRUMENT == 'hera' else 'gain'
			
			Single_LST = False  # Use only one time to test negative flux.
			if Single_LST:
				INSTRUMENT += '-sl'
			...
			baseline_safety_xx_max = 7000. if INSTRUMENT == 'hera' else 7.1 * 10. ** 10 if INSTRUMENT == 'hera-vivaldi' else 70.  # 10.**10  # Meters
			baseline_safety_yy_max = 7000. if INSTRUMENT == 'hera' else 7.1 * 10. ** 10 if INSTRUMENT == 'hera-vivaldi' else 70.  # 10.**10  # Meters
			baseline_safety_zz_max = 10. ** 10 if INSTRUMENT == 'hera' else 10. ** 5 if INSTRUMENT == 'hera-vivaldi' else 10. ** 5  # Meters
			
			# if Frequency_Select >= 168. and Frequency_Select <= 169. and INSTRUMENT == 'hera':
			# 	continue
			...
			Focus_PointSource = False if (Specific_Files and not Simulation_For_All and not Single_LST) else False if not Single_LST else False
			...
			if Focus_PointSource:
				...
				# TGSSADR J101809.2-314415 [ra: 154.53835, dec: -31.73773]; TGSSADR J071717.6-250454 [ra: 109.32351, dec: -25.0817]; TGSSADR J020012.1-305327 [ra: 30.05044, dec: -30.89106], 32: 8916;
				...
				# TGSSADR J044437.6-280950	[ra:71.15667, dec:-28.16395] Flux:47367.9 mJy, 13948.1 mJy, 64:34680; Map: 31.0332 Jy 32:8667 Map:16.5593
			...
			Add_Chunks_part = (3 if 'blender' in DATA_PATH else 2) if not Simulation_For_All else (2 if 'blender' in DATA_PATH else 2)
			...
			Time_Average_afterload = (1 if Compress_Average else 1) if not Simulation_For_All else 1
			Frequency_Average_afterload = (1 if Compress_Average else 1) if not Simulation_For_All else 1
			...
			if Simulation_For_All:
				...
				if len(sys.argv) > 10:
					...
					lsts_start = -6. if 'blender' in DATA_PATH else -6.
					lsts_end = 6. if 'blender' in DATA_PATH else 6.
			...
			Rescale_fullsim_vis = True # If rescale fullsim_vis by ratio of jansky2kevin and jansky2kelvin_beamweight
			Use_nside_bw_forFullsim = True  # Use nside_beamweight to simulatie fullsim_sim
			Inter_from_standard = False  # If to interpolate equatorial_GSM_beamweight(mfreq) from nside_standerd.
			
			INSTRUMENT = INSTRUMENT + '-nb{0}'.format(nside_beamweight)
			...
			if not Simulation_For_All:
				...
				elif (filetype == 'uvh5' and 'vivaldi' not in INSTRUMENT):
					print(1)
					for session in Observing_Session:
						print(2)
			...
			jansky2kelvin_beam = 1.e-26 * ((C / freq) ** 2 / (2. * kB)) / (4 * np.pi / (12 * nside_beamweight ** 2))  # Jansky=10^-26 * W/(m^2 * Hz) = 10^-23 * erg/(s * cm^2 * Hz)
			jansky2kelvin_multifreq_beam = 1.e-26 * ((C / flist[0]) ** 2 / (2. * kB)) / (4 * np.pi / (12 * nside_beamweight ** 2))
			...
			if not Simulation_For_All:
				...
				try:
					print('dflags_dred.flags.keys(): {0}'.format(dflags_dred[0].keys()))
				except:
					try:
						print('dflags_dred_mfreq.flags.keys(): {0}'.format(dflags_dred_mfreq[0].keys()))
					except:
						pass
			...
			if not Simulation_For_All:
				...
				if Single_LST:
					# num_lst_select_delta = 1
					
					index_single = np.where(data_lsts[0] == data_lsts[0][np.abs((Point_Source_Direction['ra'] / 15.) - data_lsts[0][tmask]).argmin()])[0]
					print('index_single: {0}'.format(index_single))
					tmask[:] = False
					tmask[index_single] = True
					print('Tmask-SingleLst: {0}'.format(tmask))
			...
			used_common_bls_red = common_bls_red[la.norm(common_bls_red, axis=-1) / (C / freq) <= 1.27 * nside_standard / baseline_safety_factor]  # [np.argsort(la.norm(common_ubls, axis=-1))[10:]]     #remove shorted 10
			...
			INSTRUMENT += '-bb{0}'.format(len(badants))
			tag += '-bb{0}'.format(len(badants))
			sys.stdout.flush()
			...
			if Num_Pol == 3:
				beam_heal_hor_z = local_beam_unpol(freq_beam)[2]
			...
			if Plot_Beam:
				plt.figure(10)
				# ind = np.where(beam_freqs == freq)[0][0]
				hp.mollview(10.0 * np.log10(beam_heal_equ_x), title='HERA Beam-East ({0}MHz, bnside={1})'.format(freq_beam, bnside),
							unit='dBi')
				plt.savefig(script_dir + '/../Output/{0}-equ-Beam-east-{1:.2f}-bnside-{2}.pdf'.format(INSTRUMENT, freq_beam, bnside))
				if Num_Pol > 1:
					hp.mollview(10.0 * np.log10(beam_heal_equ_y), title='HERA Beam-North ({0}MHz, bnside={1})'.format(freq_beam, bnside),
								unit='dBi')
					plt.savefig(script_dir + '/../Output/{0}-equ-Beam-north-{1:.2f}-bnside-{2}.pdf'.format(INSTRUMENT, freq_beam, bnside))
				if Num_Pol == 3:
					hp.mollview(10.0 * np.log10(beam_heal_equ_z), title='HERA Beam-Zenith ({0}MHz, bnside={1})'.format(freq_beam, bnside),
								unit='dBi')
					plt.savefig(script_dir + '/../Output/{0}-equ-Beam-zenith-{1:.2f}-bnside-{2}.pdf'.format(INSTRUMENT, freq_beam, bnside))
				plt.show(block=False)
			...
			equatorial_GSM_standard_mfreq = np.zeros((nf_used, len(equatorial_GSM_standard)))
			if Absolute_Calibration_dred_mfreq or PointSource_AbsCal or Synthesize_MultiFreq:
			...
			if not Inter_from_standard or nside_beamweight == nside_standard:
				...
				equatorial_GSM_beamweight_mfreq = np.zeros((nf_used, len(equatorial_GSM_beamweight))) # np.zeros(0)
				if Absolute_Calibration_dred_mfreq or PointSource_AbsCal or Synthesize_MultiFreq:
			...
			# Artifitial_uniform_sky_T = 0.05 # K
			
			if Artifitial_GSM:
				equatorial_GSM_standard_mfreq_origin = equatorial_GSM_standard_mfreq
				equatorial_GSM_standard_origin = equatorial_GSM_standard
				equatorial_GSM_standard_mfreq = np.ones_like(equatorial_GSM_standard_mfreq) * (np.mean(equatorial_GSM_standard_mfreq, axis=1) / np.mean(equatorial_GSM_standard)).reshape(equatorial_GSM_standard_mfreq.shape[0], 1) * Artifitial_uniform_sky_T
				equatorial_GSM_standard = np.ones_like(equatorial_GSM_standard) * Artifitial_uniform_sky_T
			
				# try:
				equatorial_GSM_beamweight_mfreq = np.ones_like(equatorial_GSM_beamweight_mfreq) * (np.mean(equatorial_GSM_beamweight_mfreq, axis=1)/np.mean(equatorial_GSM_beamweight)).reshape(equatorial_GSM_beamweight_mfreq.shape[0], 1) * Artifitial_uniform_sky_T
				equatorial_GSM_beamweight = np.ones_like(equatorial_GSM_beamweight) * Artifitial_uniform_sky_T
				# except:
				# 	print('equatorial_GSM_beamweight not artifitially recalculated.')
			...
			if nchunk_from_memory_calculation_full and Parallel_A_fullsky:
				...
				nchunk_A_full = np.int(A_size_memory / (memory_left * 0.9 / 3.)) + Add_Chunks
			...
			if NoA_Out_fullsky or Use_memmap_A_full:
				beam_weight, fullsim_vis, gsm_beamweighted, nside_distribution, final_index, thetas, phis, sizes, abs_thresh, npix, valid_pix_mask, valid_npix, fake_solution_map, fake_solution_map_mfreq, fake_solution, valid_pix_mask_origin, extra_valid_mask = get_A_multifreq(vs=vs, fit_for_additive=False, additive_A=None, force_recompute=False, Compute_A=False, Compute_beamweight=True,
																																																																					 ...
																																																																					 Reference_Freq_Index=None, Reference_Freq=np.array([freq for id_p in range(Num_Pol)]), equatorial_GSM_standard=equatorial_GSM_standard, equatorial_GSM_standard_mfreq=equatorial_GSM_standard_mfreq, Precision_full=Precision_full,
																																																																					 maxtasksperchild=maxtasksperchild,
																																																																					 nchunk_A_full=nchunk_A_full, Use_nside_bw_forFullsim=Use_nside_bw_forFullsim,
																																																																					 ubls=ubls, used_common_ubls=used_common_ubls, nt_used=nt_used, nside_standard=nside_standard, nside_start=nside_start, nside_beamweight=nside_beamweight, beam_heal_equ_x=beam_heal_equ_x, beam_heal_equ_y=beam_heal_equ_y,
																																																																					 beam_heal_equ_x_mfreq=None, beam_heal_equ_y_mfreq=None,
																																																																					 lsts=lsts, NoA_Out_fullsky=NoA_Out_fullsky,
																																																																					 Use_BeamWeight=Use_BeamWeight, thresh=thresh, valid_pix_thresh=valid_pix_thresh, equatorial_GSM_beamweight=equatorial_GSM_beamweight, equatorial_GSM_beamweight_mfreq=equatorial_GSM_beamweight_mfreq, A_Method_leg=A_Method_leg,
																																																																					 Num_Pol=Num_Pol, beam_heal_equ_z=beam_heal_equ_z,
				...
				A, beam_weight, fullsim_vis, gsm_beamweighted, nside_distribution, final_index, thetas, phis, sizes, abs_thresh, npix, valid_pix_mask, valid_npix, fake_solution_map, fake_solution_map_mfreq, fake_solution, valid_pix_mask_origin, extra_valid_mask = get_A_multifreq(vs=vs, fit_for_additive=False, additive_A=None, force_recompute=False, Compute_A=False, Compute_beamweight=True,
																																																																						A_path='', A_got=None, A_version=1.0, AllSky=True, MaskedSky=False, Synthesize_MultiFreq=False, flist=flist, Flist_select=None, Parallel_A=Parallel_A_fullsky,
																																																																						Reference_Freq_Index=None, Reference_Freq=np.array([freq for id_p in range(Num_Pol)]), equatorial_GSM_standard=equatorial_GSM_standard, equatorial_GSM_standard_mfreq=equatorial_GSM_standard_mfreq,
																																																																						Precision_full=Precision_full, maxtasksperchild=maxtasksperchild,
																																																																						nchunk_A_full=nchunk_A_full, Use_nside_bw_forFullsim=Use_nside_bw_forFullsim,
																																																																						ubls=ubls, used_common_ubls=used_common_ubls, nt_used=nt_used, nside_standard=nside_standard, nside_start=nside_start, nside_beamweight=nside_beamweight, beam_heal_equ_x=beam_heal_equ_x, beam_heal_equ_y=beam_heal_equ_y,
																																																																						beam_heal_equ_x_mfreq=None,
																																																																						beam_heal_equ_y_mfreq=None, lsts=lsts, NoA_Out_fullsky=NoA_Out_fullsky,
																																																																						Use_BeamWeight=Use_BeamWeight, thresh=thresh, valid_pix_thresh=valid_pix_thresh, equatorial_GSM_beamweight=equatorial_GSM_beamweight, equatorial_GSM_beamweight_mfreq=equatorial_GSM_beamweight_mfreq,
																																																																						A_Method_leg=A_Method_leg, Num_Pol=Num_Pol,
																																																																						beam_heal_equ_z=beam_heal_equ_z,
																																																																						Use_memmap_A_full=Use_memmap_A_full, A_path_full=A_path_full, Use_rotated_beampattern_as_beamweight=Use_rotated_beampattern_as_beamweight, Array_Pvec=Array_Pvec_fullsky, Return_phase=Return_phase,
																																																																						Coarse_Pixels=Coarse_Pixels, Coarse_Pixels_num=Coarse_Pixels_num, valid_pix_threshold_coarse=valid_pix_threshold_coarse)
			
			print('\nJansky2Kelvin: {0};\nJansky2Kelvin_beamweight: {1}\n'.format(jansky2kelvin, jansky2kelvin_beam))
			# Rescale_fullsim_vis = True
			if Rescale_fullsim_vis:
				fullsim_vis *= (jansky2kelvin / jansky2kelvin_beam)
			...
			if not Artifitial_GSM:
				...
				autocorr_vis = np.array([[[vs.calculate_pointsource_visibility(phis_standard[n], (np.pi / 2. - thetas_standard[n]), np.array([[0, 0, 0]]), f, None, beam_heal_equ[id_p], None, lsts) / 2. for n in range(12 * nside_standard ** 2)] for id_f, f in enumerate(Flist_select[id_p])] for id_p in range(Num_Pol)],
										dtype=Precision_full).transpose((0, 1, 3, 4, 2)).reshape(Num_Pol, len(Flist_select[id_p]) * 1 * nt_used,  12 * nside_standard ** 2).dot(equatorial_GSM_standard_origin)
			print('\nAverage Sky Noise Temperature: {0} K;\nReceiver Temperature: {1} K'.format(np.mean(autocorr_vis), T_receiver))
			
			if Artifitial_GSM:
				autocorr_vis += T_receiver
				print('Average Noise Temperature with T_receiver: {0} K'.format(np.mean(autocorr_vis)))
			...
			print('\n>>>>>>>Time used for Autocorrelation calculation: {0}s \n'.format((time.time() - timer)))
			...
			if Calculate_SimulationData_Noise:
				...
				print('\n>>>>>>>>Average of noise x: {0}\n'.format(np.mean(np.abs(noise['x']))))
				#	N_acu = {}
			...
			if nchunk_from_memory_calculation:
				...
				memory_left = np.max([(- resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024. ** 3 + virtual_memory().total / 1024. ** 3), Min_Memory_Taken])
				print('\n>>>>>>>>>>> A_size: {0} GB; Memory Left: {1} GB.'.format(A_size_memory, memory_left))
				if np.int(A_size_memory / (memory_left * 0.9 / 2.) + 1) <= 1:
					nchunk = 1
				else:
					nchunk = (np.int(A_size_memory / (memory_left * 0.9 / 3)) + Add_Chunks_part) if 'blender' in DATA_PATH else (np.int(A_size_memory / (memory_left * 0.9 / 3. + Add_Chunks_part)))
				print('>>>>>>>>>>> New nchunk from memory calculation: {0} \n'.format(nchunk))
				
				if nchunk <= 1:
					ChunkbyChunk_all = False
					nchunk = 1
					print('Use A as whole.')
					...
					for PInv_rcond in PInv_rcond_list:
						for Fine_Add_Scale in Fine_Add_Scale_list:
							...
							try:
								...
								print('Time used for inverting AtNiA: {0} seconds, {1} minutes'.format(time.time() - timer_AtNiAi, (time.time() - timer_AtNiAi) / 60.))
								if Fine_Add:
			...
			try:
				if Manual_PointSource:
					...
					for id_ps in range(num_ps_fig):
						...
						outfile_data_name = script_dir + '/../Output/Point_Spread/results_w-PS-{0}-{1:.4f}MHz-nubl{2}-nt{3}-mtb{4}-mfb{5}-tb{6}-bnside-{7}-nside_standard-{8}-rescale-{9:.3f}-Deg-unl-All-S-{10}-rec-{11}-{12}-{13:.4f}-{14:.4f}.fits'.format(tag, freq, nUBL_used, nt_used, mocal_time_bin if Absolute_Calibration_dred_mfreq else '_N', mocal_freq_bin if Absolute_Calibration_dred_mfreq else '_N', precal_time_bin if pre_calibrate else '_N', bnside, nside_standard, rescale_factor, S_type,
																																																															  rcond if Add_Rcond else 'N', id_ps, random_pix_phi_theta[id_ps, 0], random_pix_phi_theta[id_ps, 1])
			...
			try:
				...
				if FornaxA_Index.shape[0] >= 1:
					...
					Flux_FornaxA_sim_clean = np.sum(Max_FornaxA_sim_clean[Max_FornaxA_sim_clean > 0.]) / jansky2kelvin
			...
			thetas_ps, phis_ps = hpf.pix2ang(nside_standard, range(hpf.nside2npix(nside_standard)), nest=True)
			id_primary = (thetas_ps < (128. / 180. * np.pi)) * (thetas_ps > (112. / 180. * np.pi)) * (phis_ps > lsts.min() / 12. * np.pi) * (phis_ps < lsts.max() / 12. * np.pi)
			# id_primary = (thetas_ps < (128. / 180. * np.pi)) * (thetas_ps > (112. / 180. * np.pi)) * (phis_ps > -6. / 12. * np.pi) * (phis_ps < 6. / 12. * np.pi)
			...
			Discrepancy_Ratio_Stripe = sol2map(Discrepancy_Ratio[Re_Mask], valid_npix=valid_npix, npix=npix, valid_pix_mask=valid_pix_mask, final_index=final_index, sizes=sizes)
			print('\n>>>>>>>>>>>>>> Discrepancy Ratio Mean Stripe: {0}\n'.format(np.mean(Discrepancy_Ratio_Stripe[id_primary])))
			Error_Ratio_model = Sigma[np.arange(Sigma.shape[0]), np.arange(Sigma.shape[1])] ** 0.5 / np.abs(fake_solution)
			print('\n>>>>>>>>>>>>>> Error Ratio Mean: {0}; Max: {1}; Min: {2}.\n'.format(np.mean(Error_Ratio_model), np.max(Error_Ratio_model), np.min(Error_Ratio_model)))
			Error_Ratio_model_stripe = sol2map(Error_Ratio_model[Re_Mask], valid_npix=valid_npix, npix=npix, valid_pix_mask=valid_pix_mask, final_index=final_index, sizes=sizes)
			print('\n>>>>>>>>>>>>>> Error Ratio Mean Stripe: {0}; Max: {1}; Min: {2}.\n'.format(np.mean(Error_Ratio_model_stripe[id_primary]), np.max(Error_Ratio_model_stripe[id_primary]), np.min(Error_Ratio_model_stripe[id_primary])))
			
			Discrepancy_Ratio_coarse = np.abs(w_solution - fake_solution) / fake_solution
			Discrepancy_Ratio_Stripe_coarse = sol2map(Discrepancy_Ratio_coarse[Re_Mask], valid_npix=valid_npix, npix=npix, valid_pix_mask=valid_pix_mask, final_index=final_index, sizes=sizes)
			print('\n>>>>>>>>>>>>>> Discrepancy Ratio Mean Coarse: {0}; Max: {1}; Min: {2}\n'.format(np.mean(Discrepancy_Ratio_coarse), np.max(Discrepancy_Ratio_coarse), np.min(Discrepancy_Ratio_coarse)))
			print('\n>>>>>>>>>>>>>> Discrepancy Ratio Mean Stripe Coarse: {0}; Max: {1}; Min: {2}\n'.format(np.mean(Discrepancy_Ratio_Stripe_coarse[id_primary]), np.max(Discrepancy_Ratio_Stripe_coarse[id_primary]), np.min(Discrepancy_Ratio_Stripe_coarse[id_primary])))
			...
			print('>>>>>>>>>>>>>> Max of Sigma: {0}'.format(np.max(Sigma[np.arange(Sigma.shape[0]), np.arange(Sigma.shape[1])] ** 0.5)))
			print('>>>>>>>>>>>>>> Min of Sigma: {0}\n'.format(np.min(Sigma[np.arange(Sigma.shape[0]), np.arange(Sigma.shape[1])] ** 0.5)))
			try:
