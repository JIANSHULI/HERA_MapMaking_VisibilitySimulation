...
for rcondA in [1e-6]:#range(-5,5):
    for noise in [.01]:
        ...
        if os.path.isfile(AtAi_file):
            print "Reading AtAi...",
            sys.stdout.flush()
            AtAi = np.fromfile(AtAi_file, dtype = 'float32').reshape((len(A[0]), len(A[0])))
        else:
            print "Computing AtA eigen values...",
            sys.stdout.flush()

            ev, _ = la.eigh(A.transpose().dot(A))
            maxev = np.max(np.abs(ev))
            print "Computing AtAi...",
            sys.stdout.flush()
            AtAi = la.pinv(A.transpose().dot(A) + maxev * rcondA * np.identity(len(A[0])))#, rcond=rcondA)
            print "AtAi type", AtAi.dtype,
            print "Done."
            sys.stdout.flush()
            AtAi.astype('float32').tofile(AtAi_file)
        ...
        vis = vis + noise * np.mean(np.abs(vis)) * np.random.randn(len(vis))
        ...
        hpv.mollview(np.log10(np.abs(raw_solution[:12*nside**2]-equatorial_GSM)/equatorial_GSM), return_projected_map=True, min=-3, max=0,title='log10(relative error), rcond = %i, noise = %i'%(np.log10(rcondA), np.log10(noise)))
        hpv.mollview(w_solution[:12*nside**2], title='Wiener solution, rcond = %i, noise = %i'%(np.log10(rcondA), np.log10(noise)))
        hpv.mollview(np.log10(np.abs(w_solution[:12*nside**2]-equatorial_GSM)/equatorial_GSM), return_projected_map=True, min=-3, max=0,title='log10(relative error wiener), rcond = %i, noise = %i'%(np.log10(rcondA), np.log10(noise)))
